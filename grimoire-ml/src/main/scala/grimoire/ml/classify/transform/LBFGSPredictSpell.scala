...
class LBFGSPredictSpell extends Spell[Row & LogisticRegressionModel,Row] with HasOutputCol with HasFeatureCols{
  ...
  override def transformImpl(dat: Row & LogisticRegressionModel): Row = {
  ...
  override def parseJson(json: JsValue): LBFGSPredictSpell.this.type = {
    vectorizer.parseJson(json)
    super.parseJson(json)
  }
}

object LBFGSPredictSpell{
  def apply(json: JsValue): LBFGSPredictSpell = new LBFGSPredictSpell().parseJson(json)
}