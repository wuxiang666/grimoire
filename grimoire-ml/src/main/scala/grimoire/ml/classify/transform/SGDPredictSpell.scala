...
class SGDPredictSpell extends Spell[Row & LinearRegressionModel,Row] with HasOutputCol with HasFeatureCols{
  ...
  override def transformImpl(dat: Row & LinearRegressionModel): Row = {
  ...
  override def parseJson(json: JsValue): SGDPredictSpell.this.type = {
    vectorizer.parseJson(json)
    super.parseJson(json)
  }
}

object SGDPredictSpell{
  def apply(json: JsValue): SGDPredictSpell = new SGDPredictSpell().parseJson(json)
}